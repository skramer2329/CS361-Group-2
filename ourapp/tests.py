...
class TestAccountCreation(TestCase):
    ...
    def test_newUser(self):
        ...
        self.assertIn("test3@uwm.edu", self.context['accounts'], "new account not showing up in rendered response")
        ...
        r = self.client.post("/account/create", {"email": "test4@uwm.edu", "password": "pass2", "first_name": "bill",
            "last_name": "johnson", "phone_number": "(123)456-7890", "address": "123 Main St, Milwaukee, WI, 53211"}, follow=True)
        self.assertIn("test4@uwm.edu", self.context['accounts'], "new account not showing up in rendered response")

    def test_adding_existing_user(self):
        r = self.client.post("/account/create", {"email": self.instructor.email, "password": "pass3"},
                             follow=True)
